...
export default function UserManagement({ users: paginatedUsers, stats, filters }: any) {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        router.get(usersRoute.url({ search: search || undefined }), { ...filters, search }, { preserveState: true });
    };
                    ...
                    <div className="flex gap-2">
                        {[
                            ...
                            return (
                                <button
                                    key={filter.label}
                                    onClick={() => router.get(usersRoute.url({ ...filters, ...filter.params, search }), {}, { preserveState: true })}
